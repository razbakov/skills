---
description: "Daily Review (Meta-Skill) — orchestrates 8 child skills: gather browser history, AI transcripts, app usage, process Telegram inbox, sync all projects, suggest today's plan, sync to calendar, and save the review."
user_invocable: true
---
...
5. **daily-review-sync-projects** — Discover, pull, analyze all projects
6. **daily-review-suggest-plan** — Propose today's schedule with OKR view
7. **daily-review-calendar-sync** — Create Google Calendar events
8. **daily-review-save-review** — Save complete review to sessions file
...
Run steps 1-4 in parallel where possible to save time.
...
Depends on: Step 4 (Telegram items may add tasks to projects)
...
Depends on: Steps 1-5 (needs yesterday's data + project list with task IDs)
...
Depends on: Step 6 (needs approved schedule)
...
- If the user says "skip yesterday" — jump straight to Step 5
- Steps 1, 2, and 4 have no dependencies on each other and should run in parallel
- Step 3 depends on Step 1 (appends to the same file)
- Sub-agents in Step 5 should run in parallel for speed — one agent per project
- If today is Saturday, Step 6 will remind about the weekly review
- **Always print the full output (project list + OKR view) to chat.** Saving to file is not a substitute for showing the user
- Actionable items from Telegram inbox should be merged into the project list
