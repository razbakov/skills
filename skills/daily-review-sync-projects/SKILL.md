...
1. Read the **Project Path Registry** from the project's CLAUDE.md (README.md) — it maps project names to their actual git repo paths. Many repos are nested inside parent directories.
2. Also scan for any new repos not yet in the registry: `find <projects_dir> -maxdepth 4 -name ".git" -type d | grep -v .archive | grep -v node_modules`
3. Merge discovered repos with the registry. If new repos are found, update the registry in CLAUDE.md.
